...
- [x] 1.1 Create a subscription-scope Bicep entry point that creates `rg-azure-devops-release-notes` in `germanywestcentral`, accepts a globally unique naming suffix, invokes the workload module, and outputs the Static Web App name and hostname; verify with `az bicep build`.
...
- [x] 3.1 Replace README manual Azure resource creation instructions with the Bicep layout, resource group `rg-azure-devops-release-notes`, Germany West Central location, Free Static Web Apps SKU, and Cosmos free-tier serverless limitations; verify the deployment section contains each value.
